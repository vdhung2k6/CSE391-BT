...
// 5. Tổng giá trị kho
const totalInventoryValue = (products) => products.reduce((sum, p) => sum + (p.price * p.stock), 0);

// 6. Định dạng danh sách
const formatProductList = (products) => products.map(p => ({
    name: p.name,
    formattedPrice: p.price.toLocaleString() + "đ"
}));

// 7. Rating trung bình
const averageRating = (products) => products.reduce((sum, p) => sum + p.rating, 0) / products.length;
